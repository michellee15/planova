...
Each recommendation item also includes `googleMapsUrl`, a keyless Google Maps
search link generated by the backend from the OpenStreetMap place name and
address. The frontend can open this link so users can inspect the matching place
and its reviews. Because this is a search URL rather than a Google Place ID, users
should confirm that Google Maps selected the intended venue.

Historical messages created before this field was introduced are enriched when
chat history is read, so clients receive `googleMapsUrl` without rewriting the
stored JSONB records.
